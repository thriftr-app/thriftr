...
def cleanup_account(identifier: str):
    lookup_result = client.post('db/accounts/lookup', params={'identifier': identifier}).json()
    
    if lookup_result.get('found'):
        delete_result = client.post('db/accounts/delete', params={'identifier': identifier})
...
def test_dupe_username():
    ...
    first_register = client.post('auth/register', json=request_a)
    assert first_register.status_code == 200

    second_register = client.post('auth/register', json=request_b)
